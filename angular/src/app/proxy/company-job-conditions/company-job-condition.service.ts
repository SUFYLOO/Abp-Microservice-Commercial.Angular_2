...
export class CompanyJobConditionService {
  ...
  getList = (input: GetCompanyJobConditionsInput, config?: Partial<Rest.Config>) =>
    this.restService.request<any, PagedResultDto<CompanyJobConditionDto>>({
      method: 'GET',
      url: '/api/app/company-job-conditions',
      params: { filterText: input.filterText, companyMainId: input.companyMainId, companyJobId: input.companyJobId, workExperienceYearCode: input.workExperienceYearCode, educationLevel: input.educationLevel, majorDepartmentCategory: input.majorDepartmentCategory, languageCondition: input.languageCondition, computerExpertiseEtc: input.computerExpertiseEtc, professionalLicense: input.professionalLicense, professionalLicenseEtc: input.professionalLicenseEtc, workSkills: input.workSkills, workSkillsEtc: input.workSkillsEtc, drvingLicense: input.drvingLicense, etcCondition: input.etcCondition, extendedInformation: input.extendedInformation, dateAMin: input.dateAMin, dateAMax: input.dateAMax, dateDMin: input.dateDMin, dateDMax: input.dateDMax, sortMin: input.sortMin, sortMax: input.sortMax, note: input.note, status: input.status, sorting: input.sorting, skipCount: input.skipCount, maxResultCount: input.maxResultCount },
    },
    { apiName: this.apiName,...config });
