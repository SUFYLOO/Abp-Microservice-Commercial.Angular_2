...
export class CompanyJobContentService {
  ...
  getList = (input: GetCompanyJobContentsInput, config?: Partial<Rest.Config>) =>
    this.restService.request<any, PagedResultDto<CompanyJobContentDto>>({
      method: 'GET',
      url: '/api/app/company-job-contents',
      params: { filterText: input.filterText, companyMainId: input.companyMainId, companyJobId: input.companyJobId, name: input.name, jobTypeCode: input.jobTypeCode, peopleRequiredNumberMin: input.peopleRequiredNumberMin, peopleRequiredNumberMax: input.peopleRequiredNumberMax, peopleRequiredNumberUnlimited: input.peopleRequiredNumberUnlimited, jobType: input.jobType, jobTypeContent: input.jobTypeContent, salaryPayTypeCode: input.salaryPayTypeCode, salaryMinMin: input.salaryMinMin, salaryMinMax: input.salaryMinMax, salaryMaxMin: input.salaryMaxMin, salaryMaxMax: input.salaryMaxMax, salaryUp: input.salaryUp, workPlace: input.workPlace, workHours: input.workHours, workHour: input.workHour, workShift: input.workShift, workRemoteAllow: input.workRemoteAllow, workRemoteTypeCode: input.workRemoteTypeCode, workRemote: input.workRemote, workDifferentPlaces: input.workDifferentPlaces, holidaySystemCode: input.holidaySystemCode, workDayCode: input.workDayCode, workIdentityCode: input.workIdentityCode, disabilityCategory: input.disabilityCategory, extendedInformation: input.extendedInformation, dateAMin: input.dateAMin, dateAMax: input.dateAMax, dateDMin: input.dateDMin, dateDMax: input.dateDMax, sortMin: input.sortMin, sortMax: input.sortMax, note: input.note, status: input.status, sorting: input.sorting, skipCount: input.skipCount, maxResultCount: input.maxResultCount },
    },
    { apiName: this.apiName,...config });
