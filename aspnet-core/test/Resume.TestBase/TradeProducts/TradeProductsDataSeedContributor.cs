using System;
using System.Threading.Tasks;
using Volo.Abp.Data;
using Volo.Abp.DependencyInjection;
using Volo.Abp.Uow;
using Resume.TradeProducts;

namespace Resume.TradeProducts
{
    public class TradeProductsDataSeedContributor : IDataSeedContributor, ISingletonDependency
    {
        private bool IsSeeded = false;
        private readonly ITradeProductRepository _tradeProductRepository;
        private readonly IUnitOfWorkManager _unitOfWorkManager;

        public TradeProductsDataSeedContributor(ITradeProductRepository tradeProductRepository, IUnitOfWorkManager unitOfWorkManager)
        {
            _tradeProductRepository = tradeProductRepository;
            _unitOfWorkManager = unitOfWorkManager;

        }

        public async Task SeedAsync(DataSeedContext context)
        {
            if (IsSeeded)
            {
                return;
            }

            await _tradeProductRepository.InsertAsync(new TradeProduct
            (
                id: Guid.Parse("ebfa7967-d5e3-43f5-8eec-27dda7aa4e39"),
                name: "5d7d2964d0064a96af6b765967b4d0e7a1bc52cb63d54a90b3",
                contents: "",
                productCategoryCode: "1432714ba6a54ef690d049dec1db2766584e5ceec1b74127be",
                unitPrice: 487341433,
                unitPricePromotions: 1992947558,
                unitCode: "677700db56b0443d9ed8799a5dd0141feba8e1c009cc4639ac",
                quantityStock: 749143145,
                quantityOrdered: 1631727321,
                quantitySafetyStock: 1152184128,
                extendedInformation: "",
                dateA: new DateTime(2008, 3, 22),
                dateD: new DateTime(2008, 6, 12),
                sort: 2133788716,
                orderStateCode: "",
                status: "45bc9dcdd4234959b3d059aed069232db7be3b50191c4fc490"
            ));

            await _tradeProductRepository.InsertAsync(new TradeProduct
            (
                id: Guid.Parse("dce34bc6-f5f5-49e0-baa4-578f675d14ef"),
                name: "8cc140b002b44f629f887bd5cbbbf37424850e917fd94b33a1",
                contents: "",
                productCategoryCode: "645a2193f35f41b9badb8f9c3939d94e99f4722be9f34e7c8a",
                unitPrice: 2042389100,
                unitPricePromotions: 727350712,
                unitCode: "08970c9d441d4545aa330f4ed2a04e6652fc035a8d664cf2af",
                quantityStock: 608591806,
                quantityOrdered: 1269282127,
                quantitySafetyStock: 599308224,
                extendedInformation: "",
                dateA: new DateTime(2012, 5, 11),
                dateD: new DateTime(2018, 8, 26),
                sort: 1626438202,
                orderStateCode: "",
                status: "b7d91a5914ac453cbd55b55a159821deee3495a2666a4e2588"
            ));

            await _unitOfWorkManager.Current.SaveChangesAsync();

            IsSeeded = true;
        }
    }
}