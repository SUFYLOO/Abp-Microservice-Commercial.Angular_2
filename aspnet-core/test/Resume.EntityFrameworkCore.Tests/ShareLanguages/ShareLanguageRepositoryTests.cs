using Shouldly;
using System;
using System.Linq;
using System.Threading.Tasks;
using Resume.ShareLanguages;
using Resume.EntityFrameworkCore;
using Xunit;

namespace Resume.ShareLanguages
{
    public class ShareLanguageRepositoryTests : ResumeEntityFrameworkCoreTestBase
    {
        private readonly IShareLanguageRepository _shareLanguageRepository;

        public ShareLanguageRepositoryTests()
        {
            _shareLanguageRepository = GetRequiredService<IShareLanguageRepository>();
        }

        [Fact]
        public async Task GetListAsync()
        {
            // Arrange
            await WithUnitOfWorkAsync(async () =>
            {
                // Act
                var result = await _shareLanguageRepository.GetListAsync(
                    name: "5501a406c45f4c86ad90a73a01a5305f721bab82005c4cfd87",
                    extendedInformation: "",
                    note: "",
                    status: "3a4bcf67e47844a8ade4b7b050c359f3d7e89c6a587348a188"
                );

                // Assert
                result.Count.ShouldBe(1);
                result.FirstOrDefault().ShouldNotBe(null);
                result.First().Id.ShouldBe(Guid.Parse("fed32ea2-e37c-49f6-971a-1b58489c2b0f"));
            });
        }

        [Fact]
        public async Task GetCountAsync()
        {
            // Arrange
            await WithUnitOfWorkAsync(async () =>
            {
                // Act
                var result = await _shareLanguageRepository.GetCountAsync(
                    name: "a9c4c0ac37be46cbbdbd3a71a9f09c50b7d6e75082e64c98b9",
                    extendedInformation: "",
                    note: "",
                    status: "38239306cc2e4f788cd8b1f9b18817448d060b364a8740588e"
                );

                // Assert
                result.ShouldBe(1);
            });
        }
    }
}