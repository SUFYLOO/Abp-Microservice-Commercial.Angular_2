...
{
    public class EfCoreUserCompanyJobPairRepository : EfCoreRepository<ResumeDbContext, UserCompanyJobPair, Guid>, IUserCompanyJobPairRepository
    {
        public EfCoreUserCompanyJobPairRepository(IDbContextProvider<ResumeDbContext> dbContextProvider)
            : base(dbContextProvider)
        {

        }

        public async Task<List<UserCompanyJobPair>> GetListAsync(
        ...
        {
            var query = ApplyFilter((await GetQueryableAsync()), filterText, userMainId, name, pairCondition, extendedInformation, dateAMin, dateAMax, dateDMin, dateDMax, sortMin, sortMax, note, status);
            query = query.OrderBy(string.IsNullOrWhiteSpace(sorting) ? UserCompanyJobPairConsts.GetDefaultSorting(false) : sorting);
        ...
        public async Task<long> GetCountAsync(
            ...
            CancellationToken cancellationToken = default)
        {
            var query = ApplyFilter((await GetDbSetAsync()), filterText, userMainId, name, pairCondition, extendedInformation, dateAMin, dateAMax, dateDMin, dateDMax, sortMin, sortMax, note, status);
            return await query.LongCountAsync(GetCancellationToken(cancellationToken));
        }

        protected virtual IQueryable<UserCompanyJobPair> ApplyFilter(
            IQueryable<UserCompanyJobPair> query,
            string filterText,
            Guid? userMainId = null,
            string name = null,
            string pairCondition = null,
