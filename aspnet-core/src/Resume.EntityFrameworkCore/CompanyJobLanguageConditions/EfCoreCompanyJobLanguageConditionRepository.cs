...
{
    public class EfCoreCompanyJobLanguageConditionRepository : EfCoreRepository<ResumeDbContext, CompanyJobLanguageCondition, Guid>, ICompanyJobLanguageConditionRepository
    {
        public EfCoreCompanyJobLanguageConditionRepository(IDbContextProvider<ResumeDbContext> dbContextProvider)
            : base(dbContextProvider)
        {

        }

        public async Task<List<CompanyJobLanguageCondition>> GetListAsync(
        ...
        {
            var query = ApplyFilter((await GetQueryableAsync()), filterText, companyMainId, companyJobId, languageCategoryCode, levelSayCode, levelListenCode, levelReadCode, levelWriteCode, extendedInformation, dateAMin, dateAMax, dateDMin, dateDMax, sortMin, sortMax, note, status);
            query = query.OrderBy(string.IsNullOrWhiteSpace(sorting) ? CompanyJobLanguageConditionConsts.GetDefaultSorting(false) : sorting);
        ...
        public async Task<long> GetCountAsync(
            ...
            CancellationToken cancellationToken = default)
        {
            var query = ApplyFilter((await GetDbSetAsync()), filterText, companyMainId, companyJobId, languageCategoryCode, levelSayCode, levelListenCode, levelReadCode, levelWriteCode, extendedInformation, dateAMin, dateAMax, dateDMin, dateDMax, sortMin, sortMax, note, status);
            return await query.LongCountAsync(GetCancellationToken(cancellationToken));
        }

        protected virtual IQueryable<CompanyJobLanguageCondition> ApplyFilter(
            IQueryable<CompanyJobLanguageCondition> query,
            string filterText,
            Guid? companyMainId = null,
            Guid? companyJobId = null,
            string languageCategoryCode = null,
