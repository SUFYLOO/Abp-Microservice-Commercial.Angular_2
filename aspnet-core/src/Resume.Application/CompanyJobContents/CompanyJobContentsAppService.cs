...
namespace Resume.CompanyJobContents
{
    ...
    public class CompanyJobContentsAppService : ApplicationService, ICompanyJobContentsAppService
    {
        ...
        {
            var totalCount = await _companyJobContentRepository.GetCountAsync(input.FilterText, input.CompanyMainId, input.CompanyJobId, input.Name, input.JobTypeCode, input.PeopleRequiredNumberMin, input.PeopleRequiredNumberMax, input.PeopleRequiredNumberUnlimited, input.JobType, input.JobTypeContent, input.SalaryPayTypeCode, input.SalaryMinMin, input.SalaryMinMax, input.SalaryMaxMin, input.SalaryMaxMax, input.SalaryUp, input.WorkPlace, input.WorkHours, input.WorkHoursCustom, input.WorkShift, input.WorkRemoteAllow, input.WorkRemoteTypeCode, input.WorkRemoteDescript, input.BusinessTrip, input.HolidaySystemCode, input.Dispatched, input.WorkDayCode, input.WorkIdentity, input.DisabilityCategory, input.ExtendedInformation, input.DateAMin, input.DateAMax, input.DateDMin, input.DateDMax, input.SortMin, input.SortMax, input.Note, input.Status);
        ...
        public virtual async Task<CompanyJobContentDto> CreateAsync(CompanyJobContentCreateDto input)
        {
            ...
            input.CompanyMainId, input.CompanyJobId, input.Name, input.JobTypeCode, input.PeopleRequiredNumber, input.PeopleRequiredNumberUnlimited, input.JobType, input.JobTypeContent, input.SalaryPayTypeCode, input.SalaryMin, input.SalaryMax, input.SalaryUp, input.WorkPlace, input.WorkHours, input.WorkHoursCustom, input.WorkShift, input.WorkRemoteAllow, input.WorkRemoteTypeCode, input.WorkRemoteDescript, input.BusinessTrip, input.HolidaySystemCode, input.Dispatched, input.WorkDayCode, input.WorkIdentity, input.DisabilityCategory, input.ExtendedInformation, input.DateA, input.DateD, input.Sort, input.Note, input.Status
            );
